...
const Onboarding = () => {
        ...
        return (
          ...
          >
            <div className="grid gap-3">
              {['Student', 'Working Professional', 'Career Switcher', 'Government Exam Aspirant', 'Lifelong Learner'].map(role => (
                 <SelectionCard 
                   key={role} 
                   label={role} 
        ...
        return (
           ...
           >
             <div className="space-y-6">
                <div>
                   <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Current Level</label>
                   <div className="grid grid-cols-2 gap-3">
                     {['School', 'Undergraduate', 'Postgraduate', 'Self-learning'].map(level => (
                       <SelectionCard 
                         key={level} 
                         label={level} 
                         compact
                ...
                <div>
                   <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Field of Interest (Multiple)</label>
                   <div className="grid gap-3">
                     {['IT / Computer Science', 'Core Engineering', 'Government & Policy', 'Science & Research', 'Other'].map(field => (
                       <SelectionCard 
                         key={field} 
                         label={field} 
                         multi
        ...
        return (
           ...
           >
             <div className="flex flex-wrap gap-3">
                {['AI & Technology', 'Core Engineering Concepts', 'Public Policy & Governance', 'Research & Innovation', 'Problem Solving', 'General Knowledge', 'Economics', 'History'].map(interest => (
                  <button
                    key={interest}
      ...
      case 5:
         return (
             ...
             >
               <div className="space-y-6">
                 <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Pace</label>
                    <div className="grid gap-3">
                       {['Slow & Deep', 'Balanced', 'Fast & Goal-Driven'].map(pace => (
                          <SelectionCard 
                             key={pace} 
                             label={pace} 
                 ...
                 <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Daily Availability</label>
      ...
      case 6:
          return (
             <div className="text-center animate-fade-in space-y-6 py-10">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Your learning space is ready</h2>
  ...
  const isStepValid = () => {
    switch (step) {
       ...
       case 2: return !!formData.education && formData.fields.length > 0;
       case 3: return formData.interests.length > 0;
       case 4: return !!formData.goal;
       case 5: return !!formData.pace && !!formData.dailyTime;
       default: return true;
    }
  };
               ...
               {[1,2,3,4,5].map(s => (
                  <div 
                    ...
                    className={`h-1 rounded-full transition-all duration-300 ${
                       s <= step ? 'w-8 bg-black dark:bg-white' : 'w-2 bg-gray-200 dark:bg-gray-800'
                    }`}
                  />
               ))}
            </div>
...
const StepContainer = ({ title, subtitle, children }) => (
   <div className="animate-fade-in">
      <div className="mb-8 text-center md:text-left">
         <span className="text-xs font-semibold tracking-wider text-green-600 dark:text-green-500 uppercase mb-2 block">Step {title.includes('Preference') ? 5 : title.includes('Goal') ? 4 : title.includes('curious') ? 3 : title.includes('Academic') ? 2 : 1} of 5</span>
...
const SelectionCard = ({ label, selected, onClick, multi, compact }) => (
   <div 
     ...
     `}
   >
      <span className={`font-medium ${selected ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'}`}>
         {label}
      </span>
      {selected && (
